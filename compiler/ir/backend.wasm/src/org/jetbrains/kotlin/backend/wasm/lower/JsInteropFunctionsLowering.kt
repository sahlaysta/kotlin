...
class JsInteropFunctionsLowering(val context: WasmBackendContext) : DeclarationTransformer {
    ...
    override fun transformFlat(declaration: IrDeclaration): List<IrDeclaration>? {
        if (context.configuration.get(JSConfigurationKeys.WASM_TARGET, WasmTarget.WASI) == WasmTarget.WASI) return null
...
class JsInteropFunctionCallsLowering(val context: WasmBackendContext) : BodyLoweringPass {
    override fun lower(irBody: IrBody, container: IrDeclaration) {
        if (context.configuration.get(JSConfigurationKeys.WASM_TARGET, WasmTarget.WASI) == WasmTarget.WASI) return
