...
fun markExportedDeclarations(context: WasmBackendContext, irFile: IrFile, exportedFqNames: Set<FqName>) {
    val exportConstructor = when (context.configuration.get(JSConfigurationKeys.WASM_WASI_MODE, true)) {
        true -> context.wasmSymbols.wasiRelatedSymbols.wasiExportConstructor
